...
plot.effects <- function(
    inla.model, rand.effect, trans = \(x) x, 
    show.fig = TRUE,
    window = "all",...){
  
  if(rand.effect == "etaderiv" | rand.effect == "eta+deriv"){
    lagged_eta <- inla.model$summary.random$eta.2[-nrow(inla.model$summary.random$eta.2),c(1,2,3,6,5,4,7,8)]
    ...
    if(rand.effect == "eta+deriv"){ 
      spline_summary[,2:ncol(spline_summary)] <- spline_summary[,2:ncol(spline_summary)] + 
  ...
  if(window != "all" & is.numeric(window)) spline_summary <- spline_summary %>% tail(trunc(window))
  ...
  plot_data <- data.frame(
    ...
    {if(rand.effect %in% c("hour", "month")) mutate(., across(group, as.numeric)) else . }%>% 
...
simulation.plots.inla <- function(
    inla.model, data, response = "actuals.cf",
  ...
  fcst_dates <- seq(
    ...
    to = t1 %>% as.POSIXct(tz = "PST") + 24 * 60 * 60, by = "hour")
  ...
  if (is.null(sample.df)){
    ...
    if(response =="Y"){
      ...
      phi.samples <- precision.samples[,1:prec.link.npar] %*% # recover coefficients for precision model
        inla.model$.args$data$Y[2+1:prec.link.npar] %>% as.data.frame() %>% as.matrix() %>% t()
    }else{
    ...
    if (family =="beta"){
      ...
      linpredictor.samples <- sapply(windpow.samples, function(x) expit(x$latent)) %>% t()
      
      # recover alpha beta
      shape1.samp <- linpredictor.samples * phi.samples 
      shape2.samp <- -linpredictor.samples * phi.samples + phi.samples
      
      post.pred.samples <- sapply(1:nsamp, 
                                  \(x) rbeta(h, shape1.samp[x,], shape2.samp[x,])
      ) %>% t()
    } else{
      # case gaussian, no transformation
      linpredictor.samples <- sapply(windpow.samples, function(x) (x$latent)) %>% t()
      # add gaussian noise
  ...
  # Quantiles
  post.pred.quant <- apply(post.pred.samples, 2, quantile, probs = quSeq, na.rm = TRUE) %>% t()
  ...
  p.sim.small <- data %>% 
    ...
    mutate(name = factor(name, levels = rev(c(response, colnames(select.samples))))) %>% 
    ggplot(aes(time, value, col = name)) +
    geom_line() +
    coord_cartesian(...) +
    theme_bw() + xlab("date") + ylab(paste0(resp.lab," % of Capacity")) + labs(col = "") +
...
plot_actuals_model <- function(
  ...
  if(selection_method == "rank"){
    ...
    mpowrank <- rank(mpow)
    cutoffs <- seq(1,nrow(sampleS),length.out = n.sim.plot+1) %>% round(.,digits = 0)
    selection <- mapply(
      \(left,right){
        # random sample
  ...
  plot.data <- data %>%
    ...
    {if (response %in% c("err.cf","Y_err.cf"))  # add error samples on top of forecast
      mutate(.data = ., 
             across(c(
               #err.cf,
               all_of(colnames(extra.cols))), \(x) forecast.cf_orig+x ) ) else .} %>% 
    {if (response %in% c("err","Y_err"))  # add error samples on top of forecast
      mutate(.data = ., 
             across(c(
               #err.cf,
               all_of(colnames(extra.cols))), \(x) forecast+x ) ) else .}
  ...
  updated_response <- if (grepl("Y_",response)) substr(response,3,nchar(response)) else response
  model_w_actuals <- (
    plot.data %>%
      pivot_longer(cols = c(all_of(updated_response), all_of(c(fcst.name.plot,obs.name.plot)), matches(plot.type))) %>%
      mutate(name = factor(name, levels = rev(c(updated_response, fcst.name.plot,obs.name.plot, colnames(extra.cols))) %>% unique())) %>%
      {if (grepl("err",response)) filter(., !grepl("err", name)) else .} %>%  # remove observed error 
      # {if (response %in% c("err.cf","Y_err.cf")) filter(., name != "err.cf") else .} %>%  # remove error 
      {if (response %in% c("actuals","Y_actuals")) filter(., name != "actuals.cf") else .} # remove normalised
  ) %>%
    ...
    theme_bw() + 
    # xlab("date") + 
    ylab(paste0(resp.lab," % of Capacity")) + labs(col = "") +
    # scale_color_manual(values = rep("lightblue", ncol(extra.cols)+2)) +
    {if (legend.opt=="normalised") {scale_color_manual(
    ...
    labs(x = "")+ scale_x_datetime(date_labels = "%H:%M")+
    theme(
...
simulation.plots.inla2 <- function(
  ...
  t1 <- t1 %>%
    {if (is.character(.)) as.POSIXct(.,format = "%Y-%m-%d %H:%M:%S", tz = "PST") else .}
  ...
  if (is.null(sample.df)){
    ...
    if (ar.type == "AR1"){
      ...
      if (ar.type != "none"){
        # get ar par samples
        sigma_ar <- precision.samples[,"Precision for t"]
        position <- which(grepl(pattern = "^PACF[0-9]+ for t$",colnames(precision.samples)))
    ...
    if (family =="beta"){
      ...
      post.pred.samples <- sapply(
        1:nsamp, 
        \(x) rbeta(ncol(linpredictor.samples), shape1.samp[x,], shape2.samp[x,])
      ) %>% t()
  ...
  plot.q <- plot_actuals_model(
    data, samples = post.pred.samples, response = response,
    t1 = t1,
    h = h , resp.lab = resp.lab,
    plot.type = "quant",quSeq = quSeq,
  ...
  if(save.fig){
    ...
    ggsave(file.path(path, paste0(run.name,"_quantiles.eps")), plot.q$plot, width = 3.5, height = 3)
    ggsave(file.path(path, paste0(run.name,"_sim.small.eps")), p.sim.small$plot, width = 3.5, height = 3)
    ggsave(file.path(path, paste0(run.name,"_sim.large.eps")), p.sim.large$plot, width = 3.5, height = 3)
  }
  } else{
  ...
  invisible(
    list(
      samples = post.pred.samples, quantiles = plot.q$data,
      plot.q = plot.q$plot, p.sim.small = p.sim.small$plot, p.sim.large = p.sim.large$plot))
}

get_inla_formula <- function(inla_model){
  as.formula(inla_model$.args$formula %>% deparse(backtick = T) %>% paste(collapse = ""))
}
...
fit_a_date <- function(
  ...
  model_data <- dat.power %>% 
    ...
    # ) %>% 
    # set target response to NA to be able to forecast it
    mutate(across(c(actuals.cf,actuals,err.cf,err), \(x) ifelse(time < timet, x , NA)))
  # mutate(actuals.cf = ifelse(time < timet, actuals.cf,NA)) %>%
  # mutate(err.cf = ifelse(time < timet, err.cf, NA))
  ...
  if (length(inla.object$.args$family)>1){
    ...
    day.data <- with(
      ...
      list(
        ...
        w2 = c(rep(0, n), rep(-1, n))) # weights for negative part of derivative
    )
    # adding properly named response
    day.data[[response]] <- cbind(c(rep(0, n), rep(NA, n)), # fake zeros (1st likelihood)
...
scores_day_model <- function(
  ...
  result <- functions_to_scenarios(scen.tbl, function_list, probs = c(0.005,0.025,0.5,0.975)) %>% 
    mutate(
...
stats_hour_model <- function(
  ...
  scen.tbl <- apply(
    ...
    \(row) {
      ...
      if(clipping) rescaled_sample <- pmin(1, pmax(0,rescaled_sample))
      rescaled_sample
    }) %>% t()
  observed <- "actuals.cf"
  # orig.obs <- subsample %>% 
      ...
      !!!setNames(
        lapply(p.var, function(p) {
          scoringRules::vs_sample(
            ...
            dat = scen.tbl %>% as.data.frame() %>% 
              select(all_of(t.ahead)) %>% # select times
              as.matrix() %>% t(),             # Transpose for vs_sample
            p = p
          )
        }),
...
model.scoring <- function(
  ...
  return(list(global = mean.scores, day = daily.scores.model, hour = hour.scores.model))
...
model.scoring.reliability <- function(
  ...
  stats.vec <- c(
    "mean","sd","crps","energy",
    "variogram_p_0.5","variogram_p_1","variogram_p_2")
  observed <- "actuals.cf"
  ...
  daily.scores.model <- hour_stats %>% 
    ...
    summarise(
      ...
      # crps = mean(crps)
      coverage05 = mean(between(.data[[observed]], .data[[quant_low[1]]], .data[[quant_high[1]]])),
      coverage10 = mean(between(.data[[observed]], .data[[quant_low[2]]], .data[[quant_high[2]]])),
      coverage15 = mean(between(.data[[observed]], .data[[quant_low[3]]], .data[[quant_high[3]]])),
      coverage20 = mean(between(.data[[observed]], .data[[quant_low[4]]], .data[[quant_high[4]]])),
      coverage25 = mean(between(.data[[observed]], .data[[quant_low[5]]], .data[[quant_high[5]]])),
      coverage30 = mean(between(.data[[observed]], .data[[quant_low[6]]], .data[[quant_high[6]]])),
      coverage35 = mean(between(.data[[observed]], .data[[quant_low[7]]], .data[[quant_high[7]]])),
      coverage40 = mean(between(.data[[observed]], .data[[quant_low[8]]], .data[[quant_high[8]]])),
      coverage45 = mean(between(.data[[observed]], .data[[quant_low[9]]], .data[[quant_high[9]]])),
      coverage50 = mean(between(.data[[observed]], .data[[quant_low[10]]], .data[[quant_high[10]]])),
      coverage55 = mean(between(.data[[observed]], .data[[quant_low[11]]], .data[[quant_high[11]]])),
      coverage60 = mean(between(.data[[observed]], .data[[quant_low[12]]], .data[[quant_high[12]]])),
      coverage65 = mean(between(.data[[observed]], .data[[quant_low[13]]], .data[[quant_high[13]]])),
      coverage70 = mean(between(.data[[observed]], .data[[quant_low[14]]], .data[[quant_high[14]]])),
      coverage75 = mean(between(.data[[observed]], .data[[quant_low[15]]], .data[[quant_high[15]]])),
      coverage80 = mean(between(.data[[observed]], .data[[quant_low[16]]], .data[[quant_high[16]]])),
      coverage85 = mean(between(.data[[observed]], .data[[quant_low[17]]], .data[[quant_high[17]]])),
      coverage90 = mean(between(.data[[observed]], .data[[quant_low[18]]], .data[[quant_high[18]]])),
      coverage95 = mean(between(.data[[observed]], .data[[quant_low[19]]], .data[[quant_high[19]]])),
  ...
  result <- lapply(
    ...
    \(time0){
      ...
      sim.obj$samples %>%
        ...
        write.csv(
          .,
          file.path(sample.path,paste0(mod.code,time0,".csv")), row.names = FALSE)
    }
  )
...
ramps_model <- function(
  ...
  percent_below_thresholds <- function(diff_values, thresholds) {
    sapply(thresholds, function(threshold) mean(diff_values < threshold, na.rm = TRUE) * 100)
  }
  # browser()
  # read simulations
  scen.tbl <- #file.path(
...
plot_nh_ahead <- function(
    data,                        # Input dataset
    mycols = c("actuals.cf", "forecast.cf"), # Columns to be plotted
    t0,                          # Starting time (t0)
    h = 24,
    show.fig = TRUE,
    legend.position = "bottom",
  ...
  data.subset <- data %>%
    select(time, any_of(c(xvar, mycols))) %>%  # Ensure 'time' is always selected for plotting
    filter(time >= t0, time <= t0 + hours(h)) #%>%  # Filter time range from t0 to t0 + h hours
  ...
  fig <- ggplot(data.subset, aes(x = .data[[xvar]])) +
    ...
    # plot 2nd variable as forecast
    geom_line(aes(y = !!sym(mycols[2]), col = "forecast")) +
    # plot fist variable as actuals
    geom_line(aes(y = !!sym(mycols[1]), col = "observed")) +
  ...
  # Print the plot
  if(show.fig) print(fig)
  
  # Return the filtered data invisibly
  invisible(list(data = data.subset, plot = fig))
}
...
compare_inla_models <- function(models) {
  ...
  results <- lapply(names(models), function(model_name) {
    ...
    mean_log_cpo <- if (!is.null(model$cpo)) mean(log(model$cpo$cpo), na.rm = TRUE) else NA
...
extract_ar_details <- function(formula_string) {
  ...
  # browser()
  # Check for ar1
  if (grepl(ar1_pattern, formula_string %>% paste(., collapse = " "), perl = TRUE)) {
    return(1) # ar1 defaults to order = 1
  }
  
  # Check for ar and extract the order number
  ar_match <- regmatches(formula_string, regexpr(ar_pattern, formula_string, perl = TRUE))
...
get_mod_stats <- function(
  ...
  if (length(mod.obj$.args$family) == 1){
    ...
    stats = data.frame(
      ...
      mean_log_cpo = if (!is.null(mod.obj$cpo)) mean(log(mod.obj$cpo$cpo), na.rm = TRUE) else NA,
      mean_log_gcpo = if (!is.null(mod.obj$gcpo$gcpo)) mean(log(mod.obj$gcpo$gcpo), na.rm = TRUE) else NA,
      etaderiv = FALSE
    )
  } else{ # 2 likelihood models
    ...
    stats = data.frame(
      ...
      m_rand = paste(mod.obj$model.random, collapse = "\n"),
      ar_order = mod.obj$.args$formula[[3]] %>% extract_ar_details(),
      waic = if (!is.null(mod.obj$waic)) sum(mod.obj$waic$local.waic[-c(1:n)], na.rm = TRUE) else NA,
      mean_log_cpo = if (!is.null(mod.obj$cpo)) mean(log(mod.obj$cpo$cpo[-c(1:n)]), na.rm = TRUE) else NA,
      mean_log_gcpo = if (!is.null(mod.obj$gcpo$gcpo)) mean(log(mod.obj$gcpo$gcpo[-c(1:n)]), na.rm = TRUE) else NA,
      etaderiv = TRUE
    )
...
fit_inla_model <- function(
  ...
  if(model_type$family %in% c("weibull","gamma")){
    data0 <- data0 %>% 
      filter(.data[[model_type$response]]>0| is.na(.data[[model_type$response]]))
  }
  ...
  if ("etaderiv" %in% features_vec){
    ...
    # adding properly named response
    data[[updated_response]] <- cbind(c(rep(0, n), rep(NA, n)), # fake zeros (1st likelihood)
    ...
    ctrl_family <- list(
      ...
      list(#hyper = list(theta = list(initial = log(1/s^2), fixed = FALSE)),
        control.link = list(model = "default"),
    ...
    data <- data0
    family <- model_type$family
    ctrl_family <- list(beta.censor.value = cens,
                        control.link = list(model = "default"))
  }
...
extract_score_model <- function(mod.obj){
  ...
  if(length(mod.obj$.args$family)>1){
    ...
    result <- data.frame(
      waic = if (!is.null(mod.obj$waic)) sum(mod.obj$waic$local.waic[-c(1:n)], na.rm = TRUE) else NA,
      mean_log_cpo = if (!is.null(mod.obj$cpo$cpo)) mean(log(mod.obj$cpo$cpo[-c(1:n)]), na.rm = TRUE) else NA,
      mean_log_gcpo = if (!is.null(mod.obj$gcpo$gcpo)) mean(log(mod.obj$gcpo$gcpo[-c(1:n)]), na.rm = TRUE) else NA
    )
  } else{
    result <- data.frame(
...
get_regime_models <- function(
  ...
  for (k in p_groups) {
    ...
    regime_data <- history_window(
      ...
      window = 18, mask = T) %>% 
      # mask other regimes
      mutate(across(any_of(resp_columns), \(x) ifelse(.data[[group_var]] %in% k, x, NA)))
    ...
    # browser()
    regmod_name <- sprintf(model_file %>% 
                             sub("(\\.rds)$","_reg_%s\\1",.),
                           substr(k,1,1)
    )
...
hist_wdens <- function(data, response, family = "gaussian") {
  ...
  if (family %in% c("gamma", "weibull") && any(values <= 0)) {
    ...
    message(sprintf("Found %.2f%% non-positive values in '%s'. These were removed before fitting the %s distribution.",
                    prop_neg * 100, response, family))
  ...
  if (family == "beta") {
    if (any(values <= 0 | values >= 1)) {
      ...
      message(sprintf("Beta distribution only supports values strictly in (0,1). %.2f%% of values are outside this range.", prop_out * 100))
    }
  }
  ...
  if (family == "gaussian") {
    ...
    p <- p + stat_function(fun = dgamma, args = list(shape = shape, rate = rate),
    ...
    p <- p + stat_function(fun = dweibull, args = list(shape = shape, scale = scale),
    ...
    alpha <- m * ((m * (1 - m)) / v - 1)
    beta <- (1 - m) * ((m * (1 - m)) / v - 1)
    p <- p + stat_function(fun = dbeta, args = list(shape1 = alpha, shape2 = beta),
                           color = "purple", lwd = 1)
  ...
  # Final touches
  p <- p + labs(title = paste("Histogram with", family, "Density Overlay"),
...
qq_wdens <- function(data, response, family = "gaussian") {
  ...
  if (family == "beta") {
    if (any(values <= 0 | values >= 1)) {
      prop_out <- mean(values <= 0 | values >= 1)
      message(sprintf("Found %.2f%% values outside (0,1) in '%s'. These were removed before fitting the beta distribution.",
                      prop_out * 100, response))
  ...
  # Q-Q plot
  p <- ggplot(data.frame(empirical = empirical, theoretical = theoretical),
              aes(x = theoretical, y = empirical)) +
    geom_point(color = dist_color, alpha = 0.6) +
    geom_abline(intercept = 0, slope = 1, linetype = "dashed") +
    labs(title = paste(response,"Q-Q Plot against", family, "Distribution"),
...
plot_reliability <- function(
  ...
  plot.data <- global_scores %>% 
    ...
    pivot_longer(cols = coverage05:coverage95,names_to = "level", values_to = "empirical") %>% 
    ...
    mutate(
      label = sprintf(
        ...
        family),
      label2 = ifelse( family != "nonparametric",
        sprintf(
        "%s%s%s", 
        ...
        "nonpar.") %>% factor(., levels = ordered.levels)
    ) %>% 
    {if(!is.null(code_subset))filter(., 
      as.character(label2) %in% code_subset
    )else .} %>% 
    {if(pgroup) mutate(., 
                       label2 = paste(label2, power.group %>% 
                                        gsub("20%$", "", .)) %>% gsub(" NA$","",.) %>% 
                         factor(., levels = c("NPB low","NPB mid", "NPB high",
                                              "NEN low","NEN mid", "NEN high",
                                              "nonpar."))) else .}
  # browser()
  ...
  p <- plot.data %>% 
    ...
    geom_line(aes(
      ...
      # main_term %>% substr(.,1,3) %>% toupper())), 
      lwd = 0.8)+
    labs(col = "", title = "Reliability diagrams", x = "Nominal coverage", y="Empirical coverage")+
    theme(
      legend.position = "inside",
