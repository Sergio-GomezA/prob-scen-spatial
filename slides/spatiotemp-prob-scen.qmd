...
mod.temp <- readRDS(
  file.path(
    path,
    "r_err.cf_f_gaussian_eta_feat_ws.w_group-matern-ar1-etaderiv.rds"
  )
)
...
cbind(
  ...
  as.data.frame() %>%
  setNames(
    c("Intercept", "Gau prec", "Wind RW prec", "Range", "Spat sd", "Rho t", "PR effect")
  ) %>%
  kable(digits = c(2, 0, 0, 1, 2, 2, 2), format = "latex") %>%
  kable_styling(
    latex_options = "scale_down"
  )
