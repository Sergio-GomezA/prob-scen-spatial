...
format: 
  beamer:
    ...
    slide-level: 2
    split-bib: true
    output-file: "st-prob-scen_slides_26-02-12.pdf"
header-includes:
  - \usepackage{graphicx}
fontsize: 12pt
aspectratio: 169
...
- UK data:
  ...
  - Resolution: hourly
  - Locations 35 Scottish wind farms
...
::: {.column width="50%"}
\centering
\small Sample of Scottish wind farms

\includegraphics[width=\linewidth,height=5cm,keepaspectratio]{../fig/Scottish_35wfsamp_24_map.pdf}
:::
...
::: {.column width="50%"}
\centering
\small Covariance plot
```{r fig.height = 3, fig.show="hold" ,results="asis"}
p_pattern <- sprintf("^pco.*_mod_%s.pdf$", prefix)
p_plots <- list.files(path = "../fig", pattern = p_pattern,full.names = TRUE)

knitr::include_graphics(p_plots[1])
...
knitr::include_graphics(p_plots[2])
```
:::
...
## Scenarios
\centering
\small Probabilistic scenarios
```{r results="asis",  out.height="80%"}
fig_path <- "~/Documents/proj2/spatial/ST-PR-NEN/fig"

t_plot <- "2024-07-01"
scen_fig <- file.path(
  fig_path,
  sub("\\.rds$",paste0("_t",t_plot,"_sim.small.png"), model_fname)
)
knitr::include_graphics(scen_fig)
```
...
# Parametric power Curve fitting{-}
